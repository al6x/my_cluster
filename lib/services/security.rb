class Security < ClusterManagement::Service
  def install
    apply_once :install do
      ...
      ssh_config = "#{config.config_dir!}/services/security/ssh_config".to_file
      ...
      {
        "#{config.config_dir!}/services/security/id_rsa".to_file => "~/.ssh/id_rsa",
        "#{config.config_dir!}/services/security/id_rsa.pub".to_file => "~/.ssh/id_rsa.pub"
      }.each do |from, to|        
